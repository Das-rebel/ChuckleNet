"""
V6 Agent Orchestrator for Parallel Task Execution (Standalone)
...
class V6AgentOrchestrator:
    ...
    async def _update_metrics(self, agent_id: str, result: TaskResult):
        ...
        agent_tasks = [t for t in self.completed_tasks if t.task_id.startswith(agent_id.split('_')[0])]
        if agent_tasks:
            successful_tasks = sum(1 for t in agent_tasks if t.status == "completed")
            agent_perf["success_rate"] = successful_tasks / len(agent_tasks)

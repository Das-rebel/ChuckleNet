#!/usr/bin/env python3
"""
MELD Biosemotic Laughter Detection Model - IMPROVED
Better handling of class imbalance and improved prediction performance
"""
...
class MELDBiosemoticModelImproved:
    """Improved biosemotic laughter detection with better handling of class imbalance"""
    ...
    def __init__(self, meld_path):
        ...
        print(f"🎯 MELD Biosemotic Model (Improved) Initialized")
    ...
    def train_model(self, model_type='gradient_boosting', use_class_weights=True):
        """Train biosemotic model with class weighting"""
        ...
        if use_class_weights:
            ...
            print(f"  ⚖️ Class Weights: Non-Joy={weight_dict[0]:.2f}, Joy={weight_dict[1]:.2f}")
        else:
            weight_dict = None
...
def main():
    ...
    print("🎯 MELD Biosemotic Model Training and Testing (IMPROVED)")
    print("=" * 60)

    # Initialize model
    meld_path = "~/datasets/MELD"
    model = MELDBiosemoticModelImproved(meld_path)

    # Load data
    model.load_data()

    # Test multiple model types with class weights
    ...
    for model_type in model_types:
        ...
        # Train model
        train_acc = model.train_model(model_type, use_class_weights=True)
        ...
        # Save model
        output_dir = f"/Users/Subho/autonomous_laughter_prediction/meld_models/{model_type}_improved"

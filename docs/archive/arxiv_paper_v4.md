...
3. **Multi-lingual Validation:** We demonstrate cross-lingual generalization from English to Chinese with minimal degradation (9% drop vs. 81% for text-only).
4. **Tiered Quality Framework:** We introduce a gold/silver data quality tier system for scalable laughter detection.
5. **Statistical Rigor:** All improvements are validated with bootstrap resampling (p < 0.0001).
...
Previous work on laughter detection has primarily focused on [laughter] markers in text [1], acoustic features [2], and more recently, transformer-based models [3]. The MultiLinguahah corpus (arXiv:2605.06309) [5] provides a recent large-scale multilingual laughter detection benchmark demonstrating cross-lingual transfer capabilities. Our work extends these efforts by systematically evaluating cross-performer generalization.
...
### 3.1 Tiered Quality Framework

We introduce a two-tier quality system for scalable laughter detection annotation:

| Tier | Coverage | Criteria | Purpose |
|------|----------|----------|---------|
| **Gold** | 6.5% (71 videos) | Manual verification of laughter-acoustic alignment, diverse performer sampling, high-quality audio | Rigorous model training and evaluation |
| **Silver** | 60% (~420 videos) | Automated filtering (SNR, duration, language), [laughter] marker density ≥ 2 per segment | Large-scale training, statistical significance |

The gold tier provides high-confidence ground truth for model development and held-out evaluation, while the silver tier enables large-scale training with sufficient signal-to-noise ratio for laughter detection.

### 3.2 Initial Candidate Pool

We identified an initial pool of **500+ comedy video segments** across English, Chinese, and Hindi from diverse YouTube channels. This pool was designed to capture maximum diversity in:
...
- Languages (EN, ZH, HI-LATN)

### 3.3 Automated Filtering

We applied several heuristic filters to the candidate pool:

| Filter | Criterion | Purpose |
|--------|----------|---------|
| **Language Verification** | Automatic language detection (EN/ZH/HI-LATN) | Remove segments with insufficient linguistic coverage |
...
### 3.4 Manual/Semi-Automated Refinement
...
Remaining: ~120 videos
       ↓ [Manual Verification]
Gold-Standard: 71 videos (~15K utterances, ~6.5%)

Silver Tier: ~420 videos (~60%)
       ↓ [Automated Filtering]
Remaining: ~300 videos with ≥2 [laughter] markers
```
...
- **Languages:** English (~80%), Chinese (~17%), Hindi (~3% - 48 utterances, insufficient for formal evaluation)
...
**Key Finding:** Audio generalizes **2× better** than text on held-out comedians (WavLM F1: 0.280 vs Text F1: 0.152).
...
3. **Hindi Synthetic Expansion:** Generate synthetic Hindi/Hinglish laughter data using LLM-based data augmentation to enable formal cross-lingual evaluation (currently 48 utterances insufficient)
4. **MultiLinguahah Integration:** Align our pipeline with the MultiLinguahah (arXiv:2605.06309) benchmark for standardized cross-lingual laughter detection evaluation
...
[5] MultiLinguahah (2026). "MultiLinguahah: A Large-Scale Multilingual Laughter Detection Benchmark." arXiv:2605.06309.
...
*Note: Hindi data is insufficient (48 utterances, <1%) for formal cross-lingual evaluation. Hindi synthetic data generation is planned as future work.*

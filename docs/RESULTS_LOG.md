...
| 15 | **Sep 18** | same 118v, horizon curve H∈{2,3} | Tier-1 EMNLP B/I/L | Level A protocol at longer horizons; 3 arms × 15 fold-seeds × 2 horizons | paired ΔF1 (a+w − words) | H2 **+0.1024** [+0.0768,+0.1276]; H3 **+0.1054** [+0.0799,+0.1310]; all 4 gates PASS | ✅ **P2 CLOSED** — anticipation horizon extends to ≥15 s; advantage decays negligibly; product mapping: 15 s reaction-anticipation window | `results/p2_level_a/p2_horizon_curve_results.json` |
| 16 | **Sep 18** | same 118v, comedian-disjoint folds | Tier-1 EMNLP B/I/L | Level A H=1 rerun, GroupKFold grouped by comedian key (oEmbed title/channel) | paired ΔF1 (a+w − words) | **+0.1147** [+0.0887,+0.1416]; acoustic alone +0.1158 [+0.0849,+0.1476]; both PASS | ✅ **SURVIVES comedian-disjoint split** — deltas unchanged vs video-disjoint; leakage axis closed | `results/p2_level_a/p2_levela_comedian_disjoint_results.json` |
| 17 | **Sep 18** | TIC-TALK external (90 specials, 5,416×60s blocks) | their Whisper-AT laugh events | our protocol on their features: SBERT text vs pose kinematics vs both; onset target | paired ΔF1 | text 0.959; kin+text−text +0.0012 null; kin−text −0.2394; text−position +0.2692 PASS | ⚠️ protocol PORTS to second-lab data; audio claim UNTESTABLE (no audio released); their kinematic correlate fails incremental controls — validates our correlates-vs-increment methodology | `results/p2_tictalk/tictalk_external_onset_results.json` |

...
- **D-GATE-RESULT (Sep 8, 17:09 UTC — results_v21_GATE_20CURATED.json)**: v20.2 gate on 20 curated marker-rich videos PASSED end-to-end. FULL (20v, 17,790 samples, pos 7.91%): F1 0.164±0.029, PR-AUC 0.208±0.082 (2.6× the 0.079 random line), IoU-F1@0.2 0.197±0.034 — below naive 0.29. RICH (9v, 7,813 samples, pos 14.2%): F1 0.338±0.051, PR-AUC 0.376±0.035, **IoU-F1@0.2 0.407±0.063 — first weak-label result above naive 0.29 AND above the 0.3302 118v-best** (NOT directly comparable: Tier-1 labels, 9 curated videos, selection on marker density; anchor rule still applies). CONFIRMS label-noise hypothesis: cleaner label subset ≈ doubles every metric; precision flat (0.447/0.431) while recall doubled (0.147→0.296). Fold-5 anomaly in FULL (PR-AUC 0.067 < random) shows 4-video folds are too small — 620v run fixes this. **Threshold fix D-GATE-FAIL05**: full-set genuine rate is 1.16% (forensics), so FAIL line lowered 2%→0.5% (evidence-based, documented; Sep-5 disaster misalignment is now guarded by provenance, not by rate alone). v20.2d deployed (e409ee1b).

#!/usr/bin/env python3
"""V8 Complete Training - Colab Ready"""
...
for fname in HF_FILES:
    ...
    if not Path(dst).exists() or Path(dst).stat().st_size < 100:
        ...
        print(f'{fname}: exists ({Path(dst).stat().st_size//1024}KB)')

# Download data from GitHub (WORKING URLs)
print('\nDownloading data...')
for name, url in [
...
def save_results(results):
    ...
    try:
        ...
        pass
    print(f'Saved {len(results)} epochs')
...
for ep in range(20):
    ...
    for b, l in tr:
        ...
        l = l.cuda()
        opt.zero_grad()
        F.cross_entropy(m(ids, mk), l, weight=torch.tensor([1., 5.]).cuda()).backward()
        opt.step()
    m.eval()
    p, y = [], []
    with torch.no_grad():
        for b, l in va:
            p.extend(m(b['input_ids'].cuda(), b['attention_mask'].cuda()).argmax(-1).cpu())
            y.extend(l.numpy())

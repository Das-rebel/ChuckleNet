#!/usr/bin/env python3
"""P2 horizon curve — can pre-onset acoustic context predict laughter 2+ windows ahead?

Registered follow-up to P2-LEVELA (2026-09-18). Same protocol (arms, seeds,
folds, model, anti-leakage) with target shifted to window t+H, H in {2, 3}.
H=1 result exists (p2_level_a_results.json): +0.1117 F1 over words.

Gate per horizon (treatment minus baseline, video-clustered bootstrap):
CI95 lower > 0 AND mean delta > 0.02.
"""
...
def main():
    ...
    for v in vids:
        ...
        for tx in texts:
            acc = (acc + ' ' + tx).strip()
            cum.append(acc)
        per[v] = dict(n=n, cum=cum, y=y, timing=context_timing(words[v], n))

    folds = list(GroupKFold(n_splits=N_FOLDS).split(np.zeros(len(vids)), groups=np.array(vids)))
    recs = {H: {a: [] for a in ARMS} for H in HORIZONS}
    fsum = {H: {a: [] for a in ARMS} for H in HORIZONS}

    for H in HORIZONS:
        print(f'=== HORIZON {H} (target window t+{H}, {H*WIN_SEC:.0f}s ahead) ===', flush=True)
        for seed in SEEDS:
            for fold, (tri, vai) in enumerate(folds):
                ...
                for v in tr:
                    ...
                    F_a = acoustic(v)
                    F_c = np.concatenate([F_a, F_w, per[v]['timing']], 1)
                    Xtr['words_before'].append(F_w); Xtr['acoustic_before'].append(F_a)
                    Xtr['acoustic_plus_words'].append(F_c)
                    ytr_l.append(tgt)
                for v in va:
                    n = per[v]['n']
                    tgt = np.zeros(n, dtype=np.int64)
                    ...
                    Xva['acoustic_plus_words'].append(F_c)
                    yva_l.append(tgt); meta.append((v, n))
                ...
                for a in ARMS:
                    ...
                    for v, n in meta:
                        ...
                        recs[H][a].append(dict(video=v, seed=seed, fold=fold,
                                               video_f1=float(f1_score(yy, (pp > 0.5).astype(int), zero_division=0)),
                                               n_windows=int(n), pos_rate=float(yy.mean()) if n else 0.0))
                        cur += n
                    fsum[H][a].append(dict(seed=seed, fold=fold, fold_f1=float(best[0]), epoch=best[2]))
                    print(f'H={H} seed={seed} fold={fold} arm={a} foldF1={best[0]:.4f} ep={best[2]}', flush=True)

    def paired(H, a, b):
        da = {}; db = {}
        for r in recs[H][a]: da.setdefault(r['video'], []).append(r['video_f1'])
        for r in recs[H][b]: db.setdefault(r['video'], []).append(r['video_f1'])
    ...
    summary = dict(
        experiment_id='P2-HORIZON-CURVE-118V-2026-09-18',
        created_utc_epoch=int(time.time()),
        task='predict laugh presence in window t+H from context <= t; H=2 (10s), H=3 (15s)',
        prior_H1='acoustic+words - words: +0.1117 [+0.0857,+0.1390] PASS (p2_level_a_results.json)',
        overall={}, paired_tests={}, gates={})
    for H in HORIZONS:
        summary['overall'][f'H{H}'] = {}
        for a in ARMS:
            fs = [x['fold_f1'] for x in fsum[H][a]]
            vals = [r['video_f1'] for r in recs[H][a]]
            summary['overall'][f'H{H}'][a] = dict(fold_mean_f1=float(np.mean(fs)),
                                                  fold_std_f1=float(np.std(fs)),
                                                  video_mean_f1=float(np.mean(vals)))
        t1 = paired(H, 'acoustic_plus_words', 'words_before')
        t3 = paired(H, 'acoustic_before', 'words_before')
        summary['paired_tests'][f'H{H}'] = {'acoustic_plus_words_vs_words': t1, 'acoustic_vs_words': t3}
        summary['gates'][f'H{H}'] = {
    ...
    passes = [f'{H}:{g}' for H in HORIZONS for g, v in summary['gates'][f'H{H}'].items() if v['gate_pass']]
    summary['decision_gate'] = dict(gates_passed=passes,
                                    verdict=('ANTICIPATION HORIZON EXTENDS' if passes else
                                             'HORIZON DOES NOT EXTEND BEYOND H=1 WITH THESE FEATURES'))
    summary['runtime_seconds'] = float(time.time() - t0)
    out = OUT_DIR / 'p2_horizon_curve_results.json'
    out.write_text(json.dumps(summary, indent=2))
    with gzip.open(OUT_DIR / 'p2_horizon_curve_observations.json.gz', 'wt') as f:

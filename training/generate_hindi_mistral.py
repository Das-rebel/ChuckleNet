#!/usr/bin/env python3
"""
Hindi/Hinglish synthetic data generation using Mistral AI API.
Fast, clean JSON output - no thinking tags.
"""
import requests
import json
import time
...
MODEL = "mistral-small-latest"  # Fast, good for generation
OUTPUT_DIR = Path("/Users/Subho/autonomous_laughter_prediction_essential/data/synthetic_hindi_mistral")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Prompt templates - request 20 per call for speed
PROMPTS = {
...
Output format - ONLY valid JSON array wrapped in markdown code block:
```json
[
  {"hindi": "setup punchline [LAUGHTER] continuation", "trigger": position},
  ... (20 items)
]
```
...
Each line MUST have [LAUGHTER] marker AFTER the punchline word.
Output format - ONLY valid JSON array wrapped in markdown:
```json
[
  {"hindi": "setup punchline [LAUGHTER] continuation", "trigger": position},
  ... (20 items)
...
```json
[
  {"hindi": "I went to market [LAUGHTER] and everyone stared", "trigger": 5},
  ... (20 items)
]
```
Generate NOW - 20 lines:''',

    'pure_hindi': '''Generate EXACTLY 20 pure Hindi comedy lines (no English).
Each line MUST have [LAUGHTER] marker AFTER the punchline word.
Output format - ONLY valid JSON array wrapped in markdown:
```json
[
  {"hindi": "आज मैं घर गया [LAUGHTER] और सबने प्यार किया", "trigger": 4},
  ... (20 items)
]
```
Generate NOW - 20 lines:'''
...
def normalize_example(data: Dict) -> Dict:
    ...
    while True:
        ...
        laughter_pos = len(before) - 1 if before else 0
        break
...
def convert_to_training_format(examples: List[Dict], style: str) -> List[Dict]:
    ...
    for ex in examples:
        ...
        example = {
            'example_id': f"mistral_{style}_{len(training)}",
            'language': ex.get('lang', 'hi-latn'),
            'comedian_id': style,
            'show_id': 'synthetic_mistral',
            'words': words,
            'labels': labels,
            'label': 1,
...
def main():
    ...
    print("=" * 70)
    print("HINDI GENERATION USING MISTRAL AI API")
    print(f"Model: {MODEL}")
    print(f"Output: {OUTPUT_DIR}")
    print(f"Target: {sum(STYLE_TARGETS.values())} examples (20 per call)")
    print("=" * 70)
    ...
    for style, target in STYLE_TARGETS.items():
        ...
        while len(examples) < target:
            ...
            if batch:
                ...
                print(f"  No parse from batch #{batch_count}")

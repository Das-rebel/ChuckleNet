#!/usr/bin/env python3
"""
Hindi/Hinglish synthetic data generation using Groq API.
...
MODEL = "llama-3.1-8b-instant"  # Fast, good for Hindi
OUTPUT_DIR = Path("/Users/Subho/autonomous_laughter_prediction_essential/data/synthetic_hindi_groq")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Prompt templates
PROMPTS = {
    'vir_das': '''Generate EXACTLY 10 Hindi comedy lines in Vir Das style.
Each line MUST have [LAUGHTER] marker AFTER the punchline word.
Output format - ONLY valid JSON array, NO explanations:
[
...
Generate NOW:''',
    ...
    'zakir_khan': '''Generate EXACTLY 10 Hindi comedy lines in Zakir Khan storytelling style.
Each line MUST have [LAUGHTER] marker AFTER the punchline word.
...
Generate NOW:''',
    ...
    'biswa_kalyan_rath': '''Generate EXACTLY 10 witty Hindi comedy lines in Biswa Kalyan Rath style.
Each line MUST have [LAUGHTER] marker AFTER the punchline word.
Output format - ONLY valid JSON array:
[
  {"hindi": "setup punchline_word [LAUGHTER] continuation", "trigger": position, "lang": "hi-latn"},
...
Generate NOW:''',
    ...
    'generic_hinglish': '''Generate EXACTLY 10 Hinglish comedy lines (Hindi + English mix).
Each line MUST have [LAUGHTER] marker AFTER the punchline word.
Output format - ONLY valid JSON array:
[
...
Generate NOW:''',
    ...
    'pure_hindi': '''Generate EXACTLY 10 pure Hindi comedy lines (no English).
Each line MUST have [LAUGHTER] marker AFTER the punchline word.
Output format - ONLY valid JSON array:
[
...
def convert_to_training_format(examples: List[Dict], style: str) -> List[Dict]:
    ...
    for ex in examples:
        ...
        example = {
            'example_id': f"groq_{style}_{len(training)}",
            'language': ex.get('lang', 'hi-latn'),
            'comedian_id': style,
            'show_id': 'synthetic_groq',
            'words': words,
            'labels': labels,
            'label': 1,
...
def main():
    ...
    print("=" * 70)
    print("HINDI GENERATION USING GROQ API")
    print(f"Model: {MODEL}")

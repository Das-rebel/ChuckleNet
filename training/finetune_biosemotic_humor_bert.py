#!/usr/bin/env python3
"""
Fine-tune BERT on biosemotically-suitable humor dataset.

This script trains on the filtered biosemotic dataset (103,278 samples) with
enhanced features for Duchenne laughter detection, incongruity patterns, and
...
BASELINE_F1 = 0.682
BIOSEMIOTIC_BASELINE_F1 = 0.867  # From previous smoke test
DEFAULT_STATUS_EVERY_SAMPLES = 10_000
...
class DatasetSummary:
    ...
    max_word_count: int
    avg_biosemotic_viability: float
    avg_duchenne_probability: float
    avg_incongruity_score: float
...
def load_biosemotic_split(
    csv_path: Path,
    split_name: str,
    max_samples: int | None = None,
    seed: int = 42,
) -> tuple[pd.DataFrame, DatasetSummary]:
    """Load biosemotic dataset with enhanced features."""
    ...
    # Handle biosemotic features - fill missing with defaults
    biosemotic_cols = [
    ...
    for col in biosemotic_cols:
        if col not in df.columns:
            df[col] = 0.64  # Default from filtering report
        else:
            df[col] = df[col].fillna(0.64).astype(float)
    ...
    summary = DatasetSummary(
        ...
        avg_quality_score=float(df.get("quality_score", pd.Series([99.5])).mean()),
        avg_word_count=float(df.get("word_count", pd.Series([18])).mean()),
        max_word_count=int(df.get("word_count", pd.Series([0])).max()),
        avg_biosemotic_viability=float(df["biosemotic_viability_score"].mean()),
        avg_duchenne_probability=float(df["duchenne_genuine_humor_probability"].mean()),
        avg_incongruity_score=float(df["incongruity_expectation_violation_score"].mean()),
    )
    return df, summary
...
def write_markdown_report(
    ...
    improvement_baseline = test_metrics["f1"] - BASELINE_F1
    improvement_biosemotic = test_metrics["f1"] - BIOSEMIOTIC_BASELINE_F1

    lines = [
        "# Biosemotic Humor BERT Fine-Tuning Report",
        "",
        "## Overview",
        "This model was trained on a biosemotically-filtered dataset (103,278 samples) ",
        "consisting of genuine humor content: Reddit jokes (101,609) + high-quality MELD joy samples (1,669).",
        "",
        "## Environment",
        ...
        f"- Avg biosemotic viability: `{summary_lookup['train'].avg_biosemotic_viability:.4f}`",
        f"- Avg Duchenne probability: `{summary_lookup['train'].avg_duchenne_probability:.4f}`",
        f"- Avg incongruity score: `{summary_lookup['train'].avg_incongruity_score:.4f}`",
        "",
        ...
        f"- Original baseline F1: `{BASELINE_F1:.4f}`",
        f"- Biosemotic baseline F1: `{BIOSEMIOTIC_BASELINE_F1:.4f}`",
        f"- Improvement vs original: `{improvement_baseline:+.4f} ({improvement_baseline/BASELINE_F1*100:+.1f}%)`",
        f"- Improvement vs biosemotic: `{improvement_biosemotic:+.4f} ({improvement_biosemotic/BIOSEMIOTIC_BASELINE_F1*100:+.1f}%)`",
        "",
        "## Oracle Test Threshold (Analysis Only)",
    ...
    for record in training_history:
        lines.append(
            ...
            f"train_acc={record['train_accuracy']:.4f}, val_f1={record['validation_best']['f1']:.4f}, "
            f"val_recall={record['validation_best']['recall']:.4f}, val_threshold={record['validation_best']['threshold']:.2f}"
        )

    lines.extend([
        "",
        "## Biosemotic Features Impact",
        "The filtered dataset provides:",
        "- Genuine humor structure (setup/punchline, narrative reversals)",
        "- Duchenne laughter patterns from high-quality MELD joy samples",
        "- Incongruity detection via text-based semantic shifts",
        "- Theory of Mind features from multi-party dialogue",
        "",
        "## Saved Artifacts",
    ])
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("experiments/biosemotic_humor_bert_base"),
    )
    ...
    log_status(
        "STATUS 1",
        f"Biosemotic BERT training: torch={environment.torch_version}, "
        f"transformers={environment.transformers_version}, device={environment.selected_device}",
    )

    log_status("STATUS 2", "Loading biosemotic humor dataset splits")
    train_df, train_summary = load_biosemotic_split(
        args.train_csv,
        "train",
        max_samples=args.max_train_samples,
        seed=args.seed,
    )
    validation_df, validation_summary = load_biosemotic_split(
        args.validation_csv,
        "validation",
        max_samples=args.max_validation_samples,
        seed=args.seed,
    )
    test_df, test_summary = load_biosemotic_split(
        args.test_csv,
        "test",
    ...
    for summary in summaries:
        log_status(
            "STATUS 2",
            f"{summary.split}: rows={summary.rows:,}, positive={summary.positive:,}, "
            f"negative={summary.negative:,}, biosemotic_viability={summary.avg_biosemotic_viability:.4f}",
        )
    ...
    # Balanced class weights for biosemotic dataset
    ...
    log_status(
        "STATUS 3",
        f"Starting biosemotic BERT training: epochs={args.epochs}, batch_size={args.batch_size}, "
        f"steps_per_epoch={steps_per_epoch}, target_recall={args.target_recall}",
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, 1):
            ...
            if samples_processed % args.status_every_samples < len(labels):
                ...
                log_status(
                    "PROGRESS",
                    f"Epoch {epoch}/{args.epochs}: {samples_processed}/{len(train_dataset)} samples "
        ...
        if best_rank is None or current_rank > best_rank:
            ...
            if epochs_without_improvement >= args.early_stopping_patience:
                log_status(
                    "STATUS 3",
                    f"Early stopping: best epoch={best_epoch}, val_f1={best_validation_record['f1']:.4f}",
                )
                break
    ...
    test_metrics["biosemotic_baseline_f1"] = BIOSEMIOTIC_BASELINE_F1
    test_metrics["absolute_f1_gain_vs_baseline"] = test_metrics["f1"] - BASELINE_F1
    test_metrics["absolute_f1_gain_vs_biosemotic"] = test_metrics["f1"] - BIOSEMIOTIC_BASELINE_F1
    ...
    log_status(
        "STATUS 4",
        f"Biosemotic BERT evaluation: test_f1={test_metrics['f1']:.4f}, "
    ...
    log_status(
        "STATUS 5",
        f"Biosemotic BERT artifacts saved: {args.output_dir}, best_model={best_model_dir}",
    )
